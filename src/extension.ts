...
// Define types for our word lists and configuration
interface WordList {
  path: string;
  color: string;
  enabled: boolean;
...
function createDecorationOptions(color: string): vscode.DecorationRenderOptions {
  return {
...
async function loadWordLists(): Promise<void> {
  ...
  const lists: WordList[] = config.get('wordLists') || [];
...
function findMatches(text: string): WordMatch[] {
    ...
    for (const word of singleWords) {
      ...
      while ((match = regex.exec(text)) !== null) {
        ...
        const overlaps = matches.some(m => 
          m.range.contains(startPos) || m.range.contains(endPos) ||
...
function updateDecorations() {
  ...
  const matchesByList: vscode.Range[][] = [];
  ...
  for (const match of matches) {
    ...
    const overlaps = usedRanges.some(range => 
      range.contains(match.range.start) || range.contains(match.range.end) ||
...
function triggerUpdateDecorations(throttle = false) {
  ...
  if (throttle) {
    ...
    const config = vscode.workspace.getConfiguration('wordPhraseHighlighter');
    const delay: number = config.get('refreshDelay') || 100;
